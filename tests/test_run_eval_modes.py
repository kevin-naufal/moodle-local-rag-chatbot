...
class RunEvalModeConfigTest(unittest.TestCase):
    ...
    def test_default_modes_include_msmarco(self):
        self.assertEqual(
            normalize_modes(""),
            ["llm_only", "rag_ollama", "rag_bert", "rag_msmarco"],
        )

    def test_resume_only_skips_successful_rows(self):
        with tempfile.NamedTemporaryFile("w", encoding="utf-8", delete=False) as handle:
            path = handle.name
            handle.write(json.dumps({"question_id": "q1", "mode": "llm_only", "run_id": 1, "status": "error"}) + "\n")
            handle.write(json.dumps({"question_id": "q1", "mode": "rag_bert", "run_id": 1, "status": "success"}) + "\n")

        completed = load_existing_completed_keys(Path(path))

        self.assertNotIn(("q1", "llm_only", 1), completed)
        self.assertIn(("q1", "rag_bert", 1), completed)

    def test_dedupe_answer_runs_keeps_latest_row_per_planned_job(self):
        with tempfile.NamedTemporaryFile("w", encoding="utf-8", delete=False) as handle:
            path = Path(handle.name)
            handle.write(json.dumps({"question_id": "q1", "mode": "llm_only", "run_id": 1, "status": "error"}) + "\n")
            handle.write(json.dumps({"question_id": "q1", "mode": "llm_only", "run_id": 1, "status": "success"}) + "\n")
            handle.write(json.dumps({"question_id": "q1", "mode": "rag_bert", "run_id": 1, "status": "success"}) + "\n")
            handle.write(json.dumps({"question_id": "q99", "mode": "llm_only", "run_id": 1, "status": "success"}) + "\n")

        planned_keys = {("q1", "llm_only", 1), ("q1", "rag_bert", 1)}
        ...
        self.assertEqual(
            [(row["question_id"], row["mode"], row["run_id"]) for row in rows],
            [("q1", "llm_only", 1), ("q1", "rag_bert", 1)],
        )
